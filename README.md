...
A web application for tracking grocery items, expiry dates, and simple expiry-based alerts. Users can register, log in, manage items, view a dashboard with charts, export/import CSV, and adjust preferences such as dark mode
...
*Note: Create a separate empty database for automated tests if you use pytest (e.g. `grocery_tracker_test`).*
...
- `SECRET_KEY` — use a long random string for session security.
